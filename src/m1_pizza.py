...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(1, 0), rg.Point(0, -1), rg.Point(-1, 0), rg.Point(
        0, 1)]
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for point in generate_points_on_circle(circle, number_of_points):
...
def run_test_pizza():
    ...
    # Test 4
    title = 'PIZZA test 4: 200 slices, thin black lines on a yellow circle.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    for point in generate_points_on_circle(circle, number_of_slices):
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(250, 200), 160)
    circle.outline_thickness = 1
    polygon(window, circle, 15, 'black', 2)
    window.close_on_mouse_click()
